...
namespace CastFlow.Api.Repository
{
    public class UserTalentRepository : IUserTalentRepository
    {
        ...
        public async Task AddAsync(UserTalent userTalent)
        {
            if (userTalent == null)
            {
                throw new ArgumentNullException(nameof(userTalent));
            }
        ...
        public async Task AddEmailVerificationAsync(EmailVerifier emailVerifier)
        {
            ...
            {
                throw new ArgumentNullException(nameof(emailVerifier));
            }
            var oldVerifications = _context.EmailVerifiers
                .Where(e => e.Email == emailVerifier.Email && !e.IsVerified);
